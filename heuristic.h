            ...
            if ((0 == local_soln_feasible || 0 == best_soln_feasible))
            {
                for (int c = 0; c < num_clauses; c++)
                {
                    ...
                    if (org_clause_weight[c] == top_clause_weight)
                        ...
                        //     clause_weight[c] = 1;
                        // }
                        if(best_soln_feasible == 1 && local_soln_feasible == 0 && tries > 7){
                            // clause_weight[c] = 0.1 * tuned_org_clause_weight[c];
                            clause_weight[c] = tuned_org_clause_weight[c] * 0.1 * int((tries-7) / 5) > 0.5 ? 0.5 : tuned_org_clause_weight[c] * 0.1 * int((tries-7) / 5);
                        }else{
                            clause_weight[c] = 0;
                        }
        ...
        for (int c = 0; c < num_clauses; c++)
        {
            ...
            else
            {
                if ((0 == local_soln_feasible || 0 == best_soln_feasible) && num_hclauses > 0)
                {
                    ...
                    //     clause_weight[c] = 1;
                    // }
                    if(best_soln_feasible == 1 && local_soln_feasible == 0 && tries > 6){
                            // clause_weight[c] = 0.1 * tuned_org_clause_weight[c];
                             clause_weight[c] = 0.1 * int((tries-6) / 2) ;
                        }else{
                            clause_weight[c] = 0;
                        }
                }
